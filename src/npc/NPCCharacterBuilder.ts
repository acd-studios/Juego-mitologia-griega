import { Scene, Vector3, Mesh, MeshBuilder, PBRMaterial, Color3, TransformNode, Matrix } from "@babylonjs/core";
...
export class NPCCharacterBuilder {
  ...
  /**
   * Builds an articulated Hellenic NPC character with head, torso, robe tunic, and procedural idle breathing/gesture animation.
   */
  public createHellenicScholarNPC(name: string, position: Vector3): Mesh {
    ...
    tunicMat.albedoColor = new Color3(0.7, 0.55, 0.35); // Greek linen robe
    ...
    // Procedural Idle Animation (breathing & head tilt)
    let idleTime = Math.random() * 10;
    this.scene.registerBeforeRender(() => {
      idleTime += this.scene.getEngine().getDeltaTime() * 0.002;

      // Subtle breathing motion on torso and shawl
